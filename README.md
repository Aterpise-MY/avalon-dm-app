...
| `games.state` | 进行中对局的房间存档（jsonb，含秘密身份；结束或作废时置空） |

页面右上角显示 Supabase 状态：连接中、保存中、已保存或云端离线。云端暂时不可用时不阻断现场游戏，最近名单会回退存入 `window.storage` 或 `localStorage`。

**房间存档（断线续局）**：DM 误刷新之后，开局会先问「房间还开着」，可以回到刚才那一步接着打。快照同时写云端 `games.state` 与本地 `localStorage["avalon:room"]`，取 `savedAt` 较新的一份。

早期版本规定浏览器存储不保存秘密身份，这条已经放宽——为了让离线刷新也能续上局。边界是：

- 只在传阅身份、夜晚、任务、刺杀四个阶段存档；入座与配角色阶段一个字都不写
- 终局、换一局、离开房间时立即清档；超过 12 小时不再提供，云端对应的局标记为 `abandoned`
- 只用 `window.localStorage`，不走 `window.storage` 宿主桥
- 恢复时火漆重新封上，不会因为刷新把身份直接亮出来
- 传阅与夜晚阶段还没有 `gameId`，这段时间只有本地存档，换设备接不上
...
| Supabase 不可用 | 顶部显示「云端离线」，现场游戏继续运行，名单与房间存档回退浏览器存储 |
| 刷新后还有未打完的局 | 开局先显示「房间还开着」，可以回到房间或离开房间重新开始 |
| 存档超过 12 小时 | 不再提供回到房间，本地清档，云端对应的局标记为 `abandoned` |
| 云端与本地存档不一致 | 取 `savedAt` 较新的一份；云端明确查不到这一局时才丢弃本地存档 |
| 存档损坏或版本不符 | 静默丢弃，当作没有存档，不打断 DM |
...
房间存档另有三个入口：`GET /games/active` 找出还开着的房间并顺手把超时的旧局标为 `abandoned`，`PATCH /games/:id/state` 写快照（带 `status='active'` 判断，挡住迟到的写请求），`PATCH /games/:id/abandon` 主动作废。存档的打包与还原在 `room.js` 里，是纯函数，可直接单测。
